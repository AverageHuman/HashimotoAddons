...
public final class HaCameraScreen extends Screen {
    ...
    protected void init() {
        ...
        addButton(new ButtonWidget(centerX - 105, y, 210, 20, new LiteralText(waitingForCameraToggleKey ? "Press any key..." : "Change Camera Toggle Key"), button -> {
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (waitingForCameraToggleKey) {
            if (keyCode != GLFW.GLFW_KEY_ESCAPE) {
                ...
                config.cameraToggleKeyCode = keyCode;
                config.cameraToggleScanCode = scanCode;
