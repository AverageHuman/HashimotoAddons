...
public final class HaAfkFarmingDebugOverlay extends DrawableHelper {
    ...
    public static void render(MatrixStack matrices, float tickDelta) {
        ...
        client.textRenderer.drawWithShadow(matrices, "CD: " + formatCooldown(info), x + 3, y + 39, info.cooldownMillis <= 0L ? 0x55FF55 : 0xFFAA55);
    ...
    private static String formatCooldown(HaAfkFarming.MobDebugInfo info) {
        if (info.draining) {
            return "Running until 0";
        }
        if (info.cooldownMillis <= 0L) {
            return "Ready";
        }
        return String.format(java.util.Locale.ROOT, "%.1fs", Double.valueOf(info.cooldownMillis / 1000.0D));
    }
